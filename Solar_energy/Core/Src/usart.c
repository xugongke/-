...
{
		HAL_GPIO_WritePin(RS485_CTRL2_GPIO_Port, RS485_CTRL2_Pin, GPIO_PIN_SET);
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
    ...
    if (huart == &huart3 && Size > 0) 
		{
				// 把这一批数据投递到消息缓冲区（给任务处理）
        size_t sent = xMessageBufferSendFromISR(uart3Message,a7680c_rx_buf,Size,&xHigherPriorityTaskWoken);
    ...
    {
        // 更新接收长度
        USART1_Data.rx_length = Size;
				p_tx = &USART1_Data; 
        
        // 将数据结构体指针发送到队列（使用中断安全版本）
    ...
    if(huart->Instance == USART6 && Size > 0)
    {
        ...
        // 重新启动DMA+空闲中断接收,重启接收后，USART6_RX_BUF会从头开始继续接收数据
				//不用担心数据被覆盖掉，因为这是和传感器模块通信，只有主控主动给传感器发送获取数据命令才会有数据到来，传感器不会主动发
    ...
    /*---------- UART4 空闲中断处理 ----------*/
    if(huart->Instance == UART4 && Size > 0)
    {
        // 更新接收长度
        UART4_Data.rx_length = Size;
				p_tx = &UART4_Data; 
    ...
    /*---------- UART7 空闲中断处理 ----------*/
    if(huart->Instance == UART7 && Size > 0)
    {
        // 更新接收长度
        UART7_Data.rx_length = Size;
				p_tx = &UART7_Data; 
    ...
    /*---------- UART8 空闲中断处理 ----------*/
    if(huart->Instance == UART8 && Size > 0)
    {
        // 更新接收长度
        UART8_Data.rx_length = Size;
				p_tx = &UART8_Data; 
